...
正式查询前冻结了四个研究问题：有哪些新增实现；控制位于生命周期何处；fail-open/closed、状态持久性和 bypass surface 如何影响保证；证据停留在文档、实现、测试、行为测量还是生产层。完整协议见 [`05_补充性系统映射研究协议.md`](05_补充性系统映射研究协议.md)。
